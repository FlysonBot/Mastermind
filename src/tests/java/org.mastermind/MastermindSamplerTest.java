...
class MastermindSamplerTest {

    @Test
    void testReturnsSampleOfCorrectSize() {
        int[] result = MastermindSampler.getSample(6, 4, 1000);
    ...
    void testAllCodesHaveCorrectNumberOfDigits() {
        int d = 4;
        int[] result = MastermindSampler.getSample(6, d, 1000);
        for (int code : result) {
    ...
    void testAllDigitsWithinColorRange(int c, int d) {
        int[] result = MastermindSampler.getSample(c, d, 1000);
        for (int code : result) {
    ...
    void testSampleSizeZeroReturnsEmptyArray() {
        int[] result = MastermindSampler.getSample(6, 4, 0);
    ...
    void testSinglePeg() {
        int c = 6, d = 1;
        int[] result = MastermindSampler.getSample(c, d, 500);
        for (int code : result) {
    ...
    void testOnlyOneColor() {
        ...
        int d = 4;
        int[] result = MastermindSampler.getSample(1, d, 100);
        for (int code : result) {
